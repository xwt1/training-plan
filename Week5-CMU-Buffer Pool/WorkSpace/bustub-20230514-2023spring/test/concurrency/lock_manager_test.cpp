...
TEST(LockManagerTest, RowAbortTest2) {
  fmt::print(stderr, "AbortTest1: multiple X should block\n");
...
TEST(LockManagerTest, MixedTest) {
  ...
  auto txn1 = bustub->txn_manager_->Begin(nullptr, bustub::IsolationLevel::READ_UNCOMMITTED);
  // auto txn2 = bustub->txn_manager_->Begin();

  fmt::print("------\n");

  query = "delete from test_1 where x = 0;";
  bustub->ExecuteSqlTxn(query, writer, txn1);
  std::cout << "delete from test_1 where x = 0;" << std::endl;

  {
    std::string query = "SELECT * FROM test_1;";
    std::stringstream ss;
    ...
    if (txn->GetState() == TransactionState::SHRINKING) {
      std::cout << "我是SHRINKING" << std::endl;
    }
    if (txn->GetState() == TransactionState::ABORTED) {
      std::cout << "我是ABORTED" << std::endl;
    }
    CheckGrowing(txn);
  ...
  //  std::cout << "ExecuteSqlTxn(query, writer, txn2);" << std::endl;

  query = "select * from test_1;";
  bustub->ExecuteSqlTxn(query, writer, txn1);
  std::cout << "ExecuteSqlTxn(query, writer, txn1);" << std::endl;
...
// TEST(LockManagerTest, MyTableLockUpgradeAndAbortTest) {
//   LockManager lock_mgr{};
//   TransactionManager txn_mgr{&lock_mgr};
