...
auto BufferPoolManager::FetchPage(page_id_t page_id, [[maybe_unused]] AccessType access_type) -> Page * {
  ...
  for(page_id_t i = 0;i < this->pool_size_;i++){
    if(this->pages_[i].page_id_ == page_id){
      // is_page_in_buffer = true;
      page_to_find = this->page_[i];
      break;
    }
  }
  ...
  if(!this->free_list_.empty()){
    ...
    //从disk中读取数据
    DiskManager::ReadPage(page_id,this->pages_[free_frame_id].data_);
    return (this->pages_ + free_frame_id);
  }else{
    //如果没有空闲块了,就需要看能不能牺牲一块
    frame_id_t evict_frame_id = -233; 
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  std::unordered_map <int,int>::iterator it = this->page_table_.find(page_id);
  if(it != this->page_table_.end()){
    frame_id_t flush_frame_id = *it;
    DiskManager::WritePage(this->pages_[flush_frame_id].page_id_,this->pages_[flush_frame_id].data_);
...
void BufferPoolManager::FlushAllPages() {
  for(auto it = this->page_table.begin();it != page_table.end();it++){
    frame_id_t flush_frame_id = *it;
    DiskManager::WritePage(this->pages_[flush_frame_id].page_id_,this->pages_[flush_frame_id].data_);
    this->pages_[flush_frame_id].is_dirty_ = false;
  }
